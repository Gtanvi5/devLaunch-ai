...
export default function ReportClient({ reportData }: ReportClientProps) {
  const swotEntries: SwotEntry[] = reportData.swot
    ? (
        Object.entries(reportData.swot) as Array<[keyof SwotAnalysis, string[]]>
      ).map(([category, items]) => ({ category, items }))
    : [];

  const displayIdea = reportData.idea || reportData.prompt;
              ...
              >
                ...
                <ul className="space-y-3.5">
                  {items.map((item: string, i: number) => (
                    <li
                      key={i}
                      className="flex items-start gap-3 text-zinc-600 dark:text-zinc-400 leading-relaxed text-sm"
                    >
